...
namespace Hawk.ETL.Crawlers
{
    ...
    public static class XPathAnalyzer
    {
        ...
        private static HtmlNode GetPageListURLInner(HtmlNode doc)
        {
            ...
            {
                HtmlNode r = doc.ChildNodes[index];
                HtmlNode node = GetPageListURLInner(r);
        ...
        private static bool GetDiff(HtmlNode node1, HtmlNode node2, List<string> paths, bool hasAttribute = false)
        {
            ...
            if (paths.Count > 0 && (node1.XPath.Contains("#") || node2.XPath.Contains("#")))
                return false;
            bool childHasDiff = false;
            for (int index = 0; index < Math.Max(node1.ChildNodes.Count, node2.ChildNodes.Count); index++)
            {
                HtmlNode child1 = node1.ChildNodes.Count > index ? node1.ChildNodes[index] : null;
                HtmlNode child2 = node2.ChildNodes.Count > index ? node2.ChildNodes[index] : null;
            ...
            if (hasAttribute)
            {
                List<string> attributs = node1.Attributes.Select(d => d.Name).ToList();
                attributs.AddRange(node2.Attributes.Select(d => d.Name));
                List<string> att = attributs.Distinct().ToList();
        ...
        public static string GetDataFromXPath(this HtmlDocument doc, string path,bool ishtml=false)
        {
            if (!string.IsNullOrEmpty(path))
            {
                ...
                {
                    string name = XPath.GetAttributeName(last.Split('@','[')[1]);
                    if (p2.HasAttributes)
                    {
                        HtmlAttribute a = p2.Attributes.FirstOrDefault(d => d.Name == name);
                        return a?.Value.Trim();
                    }
                }
                else if (ishtml)
        ...
        {
            bool isChildContainInfo = false;
            HtmlNode node1 = nodes.First();
            HtmlNode node2 = nodes[1];
            foreach (HtmlNode nodechild1 in node1.ChildNodes)
            {
                if (nodechild1.XPath.Contains("#"))
                {
                    continue;
                }
                string path = new XPath(nodechild1.XPath).TakeOff(node1.XPath).ToString();
                List<HtmlNode> nodechild2 =
            ...
            HtmlAttribute title = node1.Attributes.FirstOrDefault(d => d.Name == "title");
            if (title != null)
            {
            ...
            foreach (HtmlAttribute attribute in node1.Attributes)
            {
                string attr1 = attribute.Value;
        ...
        {
            string r = SearchXPathInner(freeDocument, value);
            if (r == null)
                return null;
        ...
        {
            string[] words = xpath.Split(new[] {'/'}, StringSplitOptions.RemoveEmptyEntries);
            ...
            for (int i = 0; i < words.Length; i++)
            {
                ...
                if (doc != null)
                {
                    ...
                    {
                        string[] items = words[i].Split("[]".ToCharArray());
                        name = items[0];
    ...
    private static void GetTableRootProbability( IList<HtmlNode> nodes, Dictionary<string, double> dict,bool haschild)
        {
            ...
            if (haschild)
            {
                foreach (var htmlNode in nodes)
                {
                    ...
                    GetTableRootProbability(htmlNode.ChildNodes.Where(d=>d.Name.Contains("#")==false).ToList(), dict,haschild);
                    
                }
            }
          
            List<HtmlNode> avanode = nodes.ToList();
            if (avanode.Count < 3)
                return;
            ...
            double[] childCounts = avanode.Select(d => (double)d.ChildNodes.Count).ToArray();
        ...
        static string[] ignores = new string[] { "style", "script", "comment" };
        ...
        private static void GetLeafNodeCount(this HtmlNode node, ref int count)
        {
            ...
            foreach (HtmlNode r in node.ChildNodes)
            {
        ...
        public static string SearchXPath(this HtmlNode node, string keyword, bool hasAttr)
        {
            ...
            foreach (HtmlNode item in node.ChildNodes)
            {
                ...
                {
                    string res = SearchXPath(item, keyword, hasAttr);
                    if (res != null)
                        return res;
                }
        ...
        public static string SearchXPath(this HtmlDocument doc, string keyword)
        {
            ...
            string item = doc.DocumentNode.SearchXPath(keyword, false);
            if (item != null)
                return item;
        ...
        {
            string xpath = doc.SearchXPath(keyword);
            if (xpath == null)
                return null;
        ...
        {
            XPath p1 = new XPath(xpath1).TakeOff(shortv);
            XPath p2 = new XPath(xpath2).TakeOff(shortv);
            ...
            IEnumerable<CrawlItem> exists = null)
        {
            ...
            HtmlNodeCollection nodes = doc2.DocumentNode.SelectNodes(shortv);
            if(nodes==null)
                return new List<CrawlItem>();
            List<HtmlNode> nodes3 = nodes.Where(d => d.Name.Contains("#") == false).ToList();
            if (nodes3.Count > 1)
            {
            ...
            ICollection<CrawlItem> existItems = null, bool isAttrEnabled = false)
        {
            ...
            else if (existItems.Count == 1)
            {
                ...
                {
                    GetTableRootProbability(doc2.DocumentNode.SelectSingleNode(item).ChildNodes.Where(d2=> d2.Name.Contains("#") == false).ToList(), dict, false);
            ...
            {
                var dict = new Dictionary<string, double>();
                GetTableRootProbability(doc2.DocumentNode.ChildNodes.Where(d=>d.Name.Contains("#")==false).ToList(),dict,true);
                IEnumerable<KeyValuePair<string, double>> p = dict.OrderByDescending(d => d.Value);
                foreach (var keyValuePair in p)
                {
                    List<CrawlItem> items = GetDiffNodes(doc2, keyValuePair.Key, isAttrEnabled, existItems);
                    if (items.Count > 1)
                        yield return items;
                }
            }
        ...
        public static IEnumerable<List<FreeDocument>> GetDataFromMultiURL(IEnumerable<string> url)
        {
            ...
            foreach (string r in url)
            {
                HtmlDocument doc2 = GetDocumentFromURL(r);
                if (properties == null)
        ...
        public static HtmlDocument GetHtmlDocument(string url)
        {
            ...
            var helper = new HttpHelper();
            ContentType content;
            string doc2 = helper.GetHtml(httpitem, out content);
        ...
        {
            IEnumerable<List<CrawlItem>> properties = doc.SearchPropertiesSmart();
            ...
            ListType type = ListType.List)
        {
            ...
            switch (type)
            {
                case ListType.List:
                    ...
                    string item = shortv;
                    HtmlNodeCollection nodes = doc2.DocumentNode.SelectNodes(item);
