...
namespace Hawk.Standard.Utils
{
        ...
        public static T Get<T>(this IDictionary<string, object> dat, string key)
        {
            ...
            try
            {
                if (type.IsEnum)
                {
                    ...
                    var res = (T) Enum.Parse(typeof (T), s,true);
                    return (res);
                }
                item = (T) Convert.ChangeType(dat[key], typeof (T),null);
            }
            catch (Exception ex)
            {
            }
   ...
   public static T Set<T>(this IDictionary<string, object> dict, string key, T oldValue)
        ...
        public static object Set(this IDictionary<string, object> dict, string key, object oldValue, Type type = null)
        {
            ...
            if (dict.TryGetValue(key, out data))
            {
                if (type.IsEnum)
                {
                    if (data is int)
                    {
                        var index = Convert.ChangeType(data, typeof (int),null);
                        return (index);
                    }
                    else
                    {
                        var item = data.ToString();
                        var index = Enum.Parse(type, item,true);
                        return (index);
                    }
                }
                ...
                try
                {
                    value = Convert.ChangeType(data, type,null);
                }
                catch (Exception ex)
                {
        ...
        public static T[] ToArray<T>(this IList<IFreeDocument> column, string name)
        {
            ...
            {
                m[i] = (T) Convert.ChangeType(column[i][name], typeof (T),null);
            }
